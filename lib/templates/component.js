export default {
  ...
  computed: {
    ...
    adUnitPath() {
      const { networkCode, adUnit, useGlobalNetworkCode } = this;
      if(networkCode && useGlobalNetworkCode) return `/${networkCode}/${adUnit}`;
      return adUnit;
    },
    divId() {
      const { id } = this;
  ...
  render(h) {
    const { divId, style, isEmpty, classList } = this;
    let classAttr = isEmpty ? '<%= options.emptyClass %> ' + classList : classList;
